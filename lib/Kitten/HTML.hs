...
scanChar c = do
  ...
  lift . tell $ htmlEscapeString (Text.singleton c)

-- * HTML utilities.

-- | Sanitizes a string for HTML.  Replaces characters in a
-- string with '&lt;', etc.  The resulting string is
-- suitable for direct, safe injection in an HTML body or
-- attribute.
htmlEscapeString :: Text -> Text
htmlEscapeString = Text.concatMap escape
  where
  escape '"' = "&quot;"
  escape '&' = "&amp;"
  escape '<' = "&lt;"
